...
struct Option
{
    ...
    std::string Tr_path, TrS_path, Va_path, VaS_path, Va_out_path, Tr_out_path, Va_pred_path, Tr_pred_path;
...
{
    return std::string(
"usage: gbdt [<options>] <dense_validation_path> <sparse_validation_path> <dense_train_path> <sparse_train_path> <validation_output_path> <train_output_path> <validation_prediction_path> <train_prediction_path>\n"
"\n"
"options:\n"
...
Option parse_option(std::vector<std::string> const &args)
{
    ...
    opt.Va_pred_path = args[i++];
    opt.Tr_pred_path = args[i++];
...
void predict(Problem const &prob, GBDT const &gbdt, std::string const &path)
{
    FILE *f = open_c_file(path, "w");
    ...
    {
        std::vector<float> x = construct_instance(prob, i);
        float pred = gbdt.predict(x.data());
